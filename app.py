...
label{direction:rtl!important;text-align:right!important;color:#8A94A6!important;}
...
div[data-baseweb="select"]>div{background:#111318!important;border:1px solid rgba(255,255,255,0.1)!important;border-radius:9px!important;color:#EAEDF2!important;}div[data-baseweb="select"] *{color:#EAEDF2!important;}div[data-baseweb="popover"] li{background:#111318!important;color:#EAEDF2!important;}div[data-baseweb="popover"] li:hover{background:#1a1d26!important;}
.stTextInput>div>div{background:#111318!important;border:1px solid rgba(255,255,255,0.1)!important;border-radius:9px!important;}
.stTextInput>div>div>input{color:#EAEDF2!important;background:#111318!important;}
input[type="text"],input[type="email"],input[type="number"],input[type="search"]{color:#EAEDF2!important;background:#111318!important;}
input::placeholder{color:#8A94A6!important;opacity:1!important;}
textarea::placeholder{color:#8A94A6!important;opacity:1!important;}
.stTextArea>div>div{background:#111318!important;border:1px solid rgba(255,255,255,0.1)!important;border-radius:9px!important;}
.stTextArea>div>div>textarea{color:#EAEDF2!important;background:#111318!important;}
[data-baseweb="input"] input{color:#EAEDF2!important;background:#111318!important;}
[data-baseweb="textarea"] textarea{color:#EAEDF2!important;background:#111318!important;}
...
<div class="comp-row"><span class="comp-label">الترتيب</span><span class="comp-val">{rank}</span></div>
    ...
    # ── جدول البرامج المتاحة ──
    progs = progs_raw.copy()
    if not progs.empty and "uni_id" in progs.columns:
        st.markdown('<hr class="divider">', unsafe_allow_html=True)
        st.markdown('<div class="section-label">البرامج الأكاديمية</div>', unsafe_allow_html=True)
        st.markdown('<div class="section-title" style="font-size:1.2rem;margin-bottom:18px;">البرامج المتاحة للجامعات المختارة</div>', unsafe_allow_html=True)
    ...
    if st.button("اطلب مقارنة ذكية", use_container_width=True, key="btn_compare_ai"):
        ...
        st.markdown(f'<div class="ai-result"><span class="ai-result-label">المقارنة الذكية</span>{ai_result}</div>',
                    unsafe_allow_html=True)
...
# ══ رُشد ══════════════════════════════════════
    ...
    # ── التحليل السريع ──
    st.markdown('<div class="section-label">التحليل الفوري</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title" style="font-size:1.25rem;margin-bottom:18px;">التحليل السريع</div>', unsafe_allow_html=True)
        ...
        if not qm_major.strip():
            ...
            if top3:
                ...
                for i, item in enumerate(top3[:3]):
                    uid = item.get("uni_id", "")
                    name_ar = item.get("name_ar", uid)
                    reason  = item.get("reason", "")
                    fit     = item.get("fit", "مناسب")
                    fit_color = "#5B5FEE" if fit == "مناسب" else "#D4A853"
                    uni_row = unis[unis["uni_id"] == uid]
                    city_country = ""
                    sch_tag = ""
                    if not uni_row.empty:
                        r = uni_row.iloc[0]
                        city_country = f"{r.get('city','')}, {r.get('country','')}"
                        if uni_has_sch(str(r.get("scholarship",""))):
                            sch_tag = '<span class="tag tag-sch">منحة دراسية</span>'
                    with qr_cols[i]:
                        st.markdown(f"""<div class="uni-card" style="border-left-color:{fit_color};">
<div class="uni-card-title">{name_ar}</div>
<div class="uni-card-meta">{city_country}</div>
<div style="color:var(--text-2);font-size:.83rem;margin:8px 0;">{reason}</div>
<div class="uni-card-tags">{sch_tag}<span class="tag" style="background:rgba(91,95,238,0.12);color:#9EA2FF;border:1px solid rgba(91,95,238,0.2);">{fit}</span></div>
</div>""", unsafe_allow_html=True)

            if advice:
                st.markdown(f'<div class="ai-result" style="margin-top:14px;"><span class="ai-result-label">نصيحة رُشد</span>{advice}</div>', unsafe_allow_html=True)
            if missing:
                missing_html = "".join([f'<span class="tag tag-sch" style="margin:3px;">{m}</span>' for m in missing])
                st.markdown(f'<div style="margin-top:10px;"><span style="color:var(--text-2);font-size:.85rem;margin-left:8px;">قد تحتاج إلى:</span>{missing_html}</div>', unsafe_allow_html=True)
...
# ══ تحليل البيانات ════════════════════════════
    ...
    # ── Metric Cards ──
    st.markdown(f"""<div class="stat-row" style="margin-bottom:32px;">
  <div class="stat-item"><strong>{N_UNIS}</strong><small>إجمالي الجامعات</small></div>
  <div class="stat-item"><strong>{N_PROGS}</strong><small>إجمالي البرامج</small></div>
  <div class="stat-item"><strong>{with_sch}</strong><small>جامعة تقدم منحاً</small></div>
  <div class="stat-item"><strong>{N_CTRY}</strong><small>دولة خليجية</small></div>
</div>""", unsafe_allow_html=True)
    ...
    # ── Stacked Bar: توزيع المنح حسب الدولة ──
    st.markdown('<hr class="divider">', unsafe_allow_html=True)
    st.markdown('<div class="section-label">المنح الدراسية</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-title" style="font-size:1.2rem;margin-bottom:18px;">توزيع المنح حسب الدولة</div>', unsafe_allow_html=True)
    ...
    if not unis.empty and "scholarship" in unis.columns:
        ...
        for _, row in unis.iterrows():
            ...
            for cat in ["Local","GCC","International"]:
                sch_data.append({"الدولة": ctry, "نوع المنحة": cat,
        ...
        if not sch_df.empty:
            fig_sch = px.bar(
                ...
                color_discrete_map={"Local":"#5B5FEE","GCC":"#D4A853","International":"#06B6D4"}
            )
            fig_sch.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
                                  font=dict(family="Cairo",color="#8A94A6"),
                                  margin=dict(l=10,r=10,t=38,b=10),height=320,
                                  legend=dict(bgcolor="rgba(0,0,0,0)",font=dict(color="#8A94A6")))
