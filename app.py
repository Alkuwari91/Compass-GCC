...
# ─── DESIGN TOKENS ───────────────────────────────────────────────
NAV_BG   = "#1D2D44"   # dark navy blue nav/hero
SEC_BG   = "#2D4060"   # medium navy section bg
SAGE     = "#E3EEF7"   # light blue tint band
CREAM    = "#F5F5F3"   # off-white page bg  (--white)
WHITE    = "#ffffff"
INK      = "#333333"   # main text          (--black)
MUTED    = "#6B7F96"   # secondary text     (blue-gray)
TEAL     = "#81AED9"   # brand accent blue  (--blue)
...
[data-testid="stHorizontalBlock"]:first-of-type button {{
  ...
  color: rgba(227,238,247,.55) !important;
...
.stTextInput > div > div:focus-within {{ border-color: {TEAL} !important; box-shadow: 0 0 0 2px rgba(129,174,217,.15) !important; }}
.stTextInput > div > div > input {{ color: {INK} !important; font-size: 14px !important; }}
input::placeholder, textarea::placeholder {{ color: #9BAFBF !important; opacity: 1 !important; }}
.stTextArea > div > div {{
  background: {WHITE} !important; border: 1px solid {BORDER} !important; border-radius: 3px !important;
}}
...
::-webkit-scrollbar-thumb {{ background: rgba(129,174,217,.35); border-radius: 0; }}
...
def page_header(tag, title, subtitle=""):
    ...
    sub_html = f'<p style="font-size:15px;color:rgba(227,238,247,.7);margin-top:10px;">{subtitle}</p>' if subtitle else ""
    st.markdown(f"""
...
<div style="background:{NAV_BG};margin:0 -80px;padding:100px 80px 80px;direction:rtl;overflow:hidden;">
  <div style="max-width:700px;margin:0 auto;text-align:center;">
    ...
    <p style="font-size:16px;color:rgba(227,238,247,.75);line-height:1.9;
       margin:0 auto 52px;max-width:480px;">
      اكتشف الجامعات، قارن التخصصات، واتخذ قرارك التعليمي بثقة مع مستشار ذكي يتحدث العربية
    </p>
    <div style="display:inline-flex;border:1px solid rgba(227,238,247,.2);">
      <div style="padding:22px 36px;text-align:center;border-left:1px solid rgba(227,238,247,.2);">
        <div style="font-family:Syne,sans-serif;font-size:30px;font-weight:800;color:#fff;">{N_UNIS}+</div>
        <div style="font-size:12px;color:rgba(227,238,247,.55);margin-top:5px;letter-spacing:.5px;">جامعة</div>
      </div>
      <div style="padding:22px 36px;text-align:center;border-left:1px solid rgba(227,238,247,.2);">
        <div style="font-family:Syne,sans-serif;font-size:30px;font-weight:800;color:#fff;">{N_CTRY}</div>
        <div style="font-size:12px;color:rgba(227,238,247,.55);margin-top:5px;letter-spacing:.5px;">دولة خليجية</div>
      </div>
      <div style="padding:22px 36px;text-align:center;border-left:1px solid rgba(227,238,247,.2);">
        <div style="font-family:Syne,sans-serif;font-size:30px;font-weight:800;color:#fff;">{N_PROGS}+</div>
        <div style="font-size:12px;color:rgba(227,238,247,.55);margin-top:5px;letter-spacing:.5px;">برنامج</div>
      </div>
      <div style="padding:22px 36px;text-align:center;">
        ...
        <div style="font-size:12px;color:rgba(227,238,247,.55);margin-top:5px;letter-spacing:.5px;">منحة متاحة</div>
      </div>
    </div>
  </div>
</div>
...
<div style="text-align:center;direction:rtl;">
  ...
  <p style="font-size:13.5px;color:#9BAFBF;">اكتب اسم جامعة أو دولة، أو استخدم الفلاتر أعلاه</p>
</div>""", unsafe_allow_html=True)
        v_space(60)
    else:
        f = unis.copy()
        ...
        if f.empty:
            ...
            for _, row in f.head(30).iterrows():
                ...
                t_style = f"background:rgba(29,45,68,.08);color:{MUTED};" if is_pub else f"background:rgba(29,45,68,.05);color:{INK};"
                ...
                if not progs.empty:
                    for lg in progs[progs["uni_id"]==str(row["uni_id"])]["language"].dropna().unique()[:2]:
                        langs += f'<span style="padding:2px 10px;font-size:11px;font-weight:600;background:rgba(129,174,217,.1);color:{MUTED};margin-right:6px;">{lg}</span>'
                links = ""
                if str(row.get("website","")).strip():
                    links += f'<a href="{row["website"]}" target="_blank" style="font-size:12.5px;font-weight:600;color:{TEAL};text-decoration:none;padding:5px 14px;border:1px solid rgba(129,174,217,.3);white-space:nowrap;margin-right:8px;">الموقع الرسمي</a>'
...
<div style="background:{WHITE};border-bottom:1px solid {BORDER};padding:18px 0;
  ...
  <div style="flex:1;min-width:0;">
    <div style="font-size:14.5px;font-weight:700;color:{INK};margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
      {row["name_ar"]}
      <span style="font-weight:400;color:#9BAFBF;font-size:12.5px;"> — {row["name_en"]}</span>
    </div>
    <div style="font-size:12.5px;color:#9BAFBF;margin-bottom:10px;">{row["city"]}, {row["country"]}</div>
...
<div style="background:{WHITE};border:1px solid {BORDER};border-top:3px solid {NAV_BG};padding:22px 20px;direction:rtl;">
  ...
  <div style="font-size:13px;color:#9BAFBF;margin-bottom:4px;text-align:right;">{row['city']}, {row['country']}</div>
...
<div style="background:{WHITE};border:1px solid {BORDER};border-top:3px solid {NAV_BG};padding:20px;direction:rtl;">
  ...
  <div style="font-size:12px;color:#9BAFBF;margin-bottom:10px;">{cc}</div>
  <div style="font-size:13px;color:{MUTED};line-height:1.75;margin-bottom:12px;">{rsn}</div>
  <div>{st_tag}<span style="padding:2px 10px;font-size:11px;font-weight:600;background:rgba(29,45,68,.07);color:{MUTED};">{fit}</span></div>
</div>""", unsafe_allow_html=True)
            if advice:
            ...
            if missing:
                ...
                st.markdown(f'<div style="margin-top:10px;direction:rtl;"><span style="color:#9BAFBF;font-size:13px;margin-left:8px;">قد تحتاج إلى:</span>{mh}</div>', unsafe_allow_html=True)
...
<div style="background:{SAGE};margin:0 -80px;padding:0 80px;direction:rtl;overflow:hidden;">
  <div style="display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid rgba(29,45,68,.08);">
    <div style="padding:32px 0;text-align:center;border-left:1px solid rgba(29,45,68,.08);">
...
<div style="background:{NAV_BG};margin:0 -80px;padding:72px 80px 64px;direction:rtl;overflow:hidden;">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
    <div style="background:rgba(255,255,255,.05);padding:24px 18px;border-top:2px solid {TEAL};direction:rtl;">
      ...
      <div style="font-size:13px;color:rgba(227,238,247,.65);text-align:right;">تبسيط القرار التعليمي</div>
    </div>
    <div style="background:rgba(255,255,255,.05);padding:24px 18px;border-top:2px solid {TEAL};direction:rtl;">
      ...
      <div style="font-size:13px;color:rgba(227,238,247,.65);text-align:right;">عرض الخيارات دون تحيّز</div>
    </div>
    <div style="background:rgba(255,255,255,.05);padding:24px 18px;border-top:2px solid {TEAL};direction:rtl;">
      ...
      <div style="font-size:13px;color:rgba(227,238,247,.65);text-align:right;">فهم الذات قبل التخصص</div>
    </div>
    <div style="background:rgba(255,255,255,.05);padding:24px 18px;border-top:2px solid {TEAL};direction:rtl;">
      ...
      <div style="font-size:13px;color:rgba(227,238,247,.65);text-align:right;">AI في خدمة التعليم</div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
